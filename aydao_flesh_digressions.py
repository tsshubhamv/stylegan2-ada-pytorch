...
#   ~~~ aydao ~~~~ 2020 ~~~
#   ~~~ updated for pytorch Derrick Schultz 2021 ~~~~
#
...
def generate_from_generator_adaptive(psi,radius_large,radius_small,step1,step2,video_length,seed,G):
    # psi = args.psi # 0.7
    ...
    rnd = np.random
    latents_a = rnd.randn(1, G.input_shape[1])
    latents_b = rnd.randn(1, G.input_shape[1])
...
def main(pkl,psi,radius_large,radius_small,step1,step2,seed,video_length=1.0):
    ...
    with dnnlib.util.open_url(pkl) as f:
        # generator, discriminator, g_ema = legacy.load_network_pkl(f)
        G = legacy.load_network_pkl(f)['g_ema']

    frames = generate_from_generator_adaptive(psi,radius_large,radius_small,step1,step2,video_length,seed, G)
